...
Après avoir défini les décisions admissibles, nous déterminons maintenant le
coût minimal que peut atteindre un groupe d'opérateurs coopérant seul. Nous
fixons d'abord une coalition afin de résoudre son problème opérationnel, puis
le même calcul sera répété pour tous les groupes pertinents. Cette étape ne
vise donc pas à choisir la coalition finale, mais à quantifier les différentes
possibilités de coopération qui seront comparées dans l'analyse économique.
...
est en veille. La répartition du trafic reste représentée par le vecteur
global \(\mathbf t\in\mathbb R_+^N\) défini dans la partie précédente ;
ses composantes hors de \(S\) sont imposées nulles.
...
C\bigl(S,G,\mathbf t^g(S,G)\bigr),
\]
sans présumer l'unicité de l'ensemble optimal, et
\[
    \mathbf t^*(S):=\mathbf t^g\bigl(S,G^*(S)\bigr)
\]
désigne la répartition de trafic associée à ce représentant.
...
de gardiens requiert \(O(s\,2^s)\) opérations arithmétiques et
comparaisons. Le calcul des valeurs $C^*(S)$ pour toutes les
$S \in \mathcal S$ requiert \(O(n\,3^n)\) telles opérations.
\end{proposition}

\begin{proof}
Après avoir ordonné les membres de \(S\) par coût variable unitaire, une
coalition de taille \(s\) admet \(2^s-1\) ensembles de gardiens, chacun
...
\subsection{Optimisation sur une fenêtre}
\label{subsec:optimisation_fenetre}

Considérons la fenêtre \(H\) définie à la
Sous-section~\ref{subsec:fenetre_decision}. Pour un ensemble
\(G\in\mathcal G_H(S)\) fixé, la Proposition~\ref{pro:opt} s'applique
séparément à chaque heure. En notant \(\mathbf t^{g,h}(S,G)\) la répartition
gloutonne correspondante, le coût optimal de la coalition sur la fenêtre est
\begin{equation}
\begin{aligned}
    C_H^*(S)
    &:=
    ...
    \sum_{h\in H}
    C_h\bigl(S,G,\mathbf t^{g,h}(S,G)\bigr).
    \label{eq:cout_persistant}
\end{aligned}
\end{equation}
Cette expression n'ajoute aucune composante au coût \(C\) : elle somme les
coûts horaires sous la contrainte que \(G\) reste inchangé. Un représentant
optimal est noté \(G_H^*(S)\), et les répartitions associées sont notées
\(\mathbf t^{h,*}(S)\).

À titre de comparaison, autoriser un nouvel ensemble de gardiens à chaque
heure donne
\begin{equation}
    \underline C_H^*(S)
    :=
...
\end{equation}
Cette borne inférieure décrit une politique différente, avec réoptimisation
horaire. L'énumération exacte de \(C_H^*(S)\) procède comme précédemment :
chaque \(G\subseteq S\) est testé sur toutes les heures, puis son coût horaire
optimal est sommé. Sa complexité est
\(O(|H|\,n\,3^n)\) pour l'ensemble des coalitions.
