...
L'existence d'une solution optimale pour chaque coalition a été établie
dans les parties précédentes et, d'après la Proposition~\ref{pro:opt}, la
répartition gloutonne est optimale pour tout ensemble de gardiens fixé.
Il existe donc des configurations de gardiens optimales
$G_{S}^{*}\subseteq S$ et $G_{Q}^{*}\subseteq Q$ telles que :
\begin{equation}
...
large, mais ne garantit ni un gain collectif strict ni l'existence d'une
répartition des économies qui soit individuellement acceptable pour tous
les opérateurs. La preuve repose sur l'additivité des capacités et des
coûts : les configurations optimales de $S$ et $Q$ peuvent être réunies
sans modifier les charges de leurs gardiens. Une condition suffisante pour
obtenir un gain collectif strict est étudiée ci-dessous.
\end{remarque}
...
c'est-à-dire que la demande agrégée peut être intégralement acheminée par
n'importe quel opérateur du site pris individuellement.

Cette hypothèse constitue une condition suffisante volontairement forte.
Sa pertinence empirique dépend à la fois du niveau de trafic agrégé, de
l'hétérogénéité des capacités effectives et de la définition retenue pour
le taux de charge. Elle sera donc examinée dans la partie consacrée à
l'évaluation numérique, plutôt que supposée vérifiée de manière générale.

\begin{proposition}[Stricte sous-additivité en régime de très faible trafic]
\label{prop:sousadd_stricte_faible_trafic}
Sous l'hypothèse précédente, soient $G_S^*$ et $G_Q^*$ des ensembles de
gardiens optimaux pour $S$ et $Q$, et soit
$$
\alpha\in\operatorname*{arg\,min}_{g\in G_S^*\cup G_Q^*}\gamma_g.
$$
Alors
$$
C^*(S)+C^*(Q)-C^*(S\cup Q)
\geqslant
\sum_{i\in(G_S^*\cup G_Q^*)\setminus\{\alpha\}}F_i
>0.
$$
...
\begin{proof}
Reprenons les notations de la proposition et posons :
$$C^*(S) = C(S, G_{S}^{*}, {\mathbf t}^{(S)}) \quad \text{et} \quad C^*(Q) = C(Q, G_{Q}^{*}, {\mathbf t}^{(Q)})$$
où ${\mathbf t}^{(S)}:=\mathbf t^g(S,G_S^*)$ et
${\mathbf t}^{(Q)}:=\mathbf t^g(Q,G_Q^*)$.

Par hypothèse de faible trafic, $d(S \cup Q)\leqslant q_\alpha$, ce qui
...
parmi les gardiens de $G_S^*\cup G_Q^*$, on a
$\gamma_i-\gamma_\alpha\geqslant0$ pour tout
$i\in G_S^*\cup G_Q^*$. Les deux premières sommes sont donc positives ou
nulles.
