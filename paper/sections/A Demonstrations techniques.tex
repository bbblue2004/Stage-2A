...
Fixons \(S\in\mathcal S\). À chaque \(h\in H\), puisque tous les équipements
ont la capacité \(q\), le nombre minimal de gardiens est
\(\lceil d^h(S)/q\rceil\). Tous les coûts fixes étant égaux à \(F>0\), une
solution horaire optimale active exactement ce nombre d'équipements. De plus,
pour toute famille de répartitions faisables,
\[
    \sum_{h\in H}\sum_{i\in G_h}\gamma t_i^h
...
\[
    C_H^*(S)=F\sum_{h\in H}
    \left\lceil\frac{d^h(S)}q\right\rceil+\gamma D(S)
    =FK_H(S)+\gamma D(S).
\]
...
\]
et la somme sur les heures donne \eqref{eq:cout_tres_faible_trafic}.
