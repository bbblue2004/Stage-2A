...
comptable plus complète. Notons \(C_i^{\mathrm{inv}}\) le montant total des
coûts supportés par l'opérateur \(i\) indépendamment de l'état de son équipement.
...
\]
Si \(R_i\) désigne le revenu total de l'opérateur sur la période, le
fonctionnement autonome engendre le coût comptable complet
...
\(\sum_{i\in S}R_i\) et les coûts invariants
...
est un vecteur global $\mathbf t=(t_1,\dots,t_n)\in\mathbb R_+^n$, où
...
\begin{aligned}
    &t_i=0, &&\forall i\in S\setminus G,\\
    &0\leqslant t_i\leqslant q_i, &&\forall i\in G,\\
    &\sum_{i\in S}t_i=d(S),\\
    &t_i=0, &&\forall i\in N\setminus S
\end{aligned}
\right\}.
\]
\end{definition}
